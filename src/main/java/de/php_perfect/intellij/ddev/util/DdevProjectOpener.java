...
public final class DdevProjectOpener {
    ...
    public static void openInCurrentWindow(@NotNull String path) {
        ProjectUtil.openOrImport(Path.of(path), OpenProjectTask.build().withForceOpenInNewFrame(false));
    }

    public static void openInNewWindow(@NotNull String path) {
        ProjectUtil.openOrImport(Path.of(path), OpenProjectTask.build().withForceOpenInNewFrame(true));
    }
}
